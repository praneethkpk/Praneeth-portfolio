...
export function Footer() {
  return (
    <footer className="w-full py-16 bg-surface-container-lowest border-t border-outline-variant/20">
      <div className="flex flex-col items-center gap-6 max-w-container-max mx-auto px-6">
        {/* Brand */}
        <a
          href="#home"
        ...
        <div className="flex items-center gap-6">
          {socialIcons.map(({ label, href, icon: Icon }) => (
            <a
              ...
              className="flex items-center gap-2 text-on-surface-variant hover:text-primary transition-all duration-200 hover:-translate-y-[2px] text-sm"
              aria-label={label}
            >
        ...
        <div className="flex flex-col items-center gap-3 mt-4 pt-6 border-t border-outline-variant/10 w-full max-w-md">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {[
              ...
              <span
                key={topic}
                className="px-3 py-1 text-xs font-mono text-on-surface-variant border border-outline-variant/20 rounded-full bg-surface-container hover:border-primary/30 hover:text-primary transition-colors"
              >
                {topic}
              </span>
            ))}
        ...
        <p className="text-sm text-on-surface-variant/60 mt-4 text-center">
          © {new Date().getFullYear()} Praneeth Kumar Parepalli. Built with
